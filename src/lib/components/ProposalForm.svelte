...
<form class="mt-6 flex flex-col items-center" on:submit|preventDefault={onSubmit}>
	<div class="mb-6">
		...
		<input
			...
			bind:value={words}
			class="block p-3 w-full text-gray-900 bg-gray-50 rounded-lg border border-gray-300 sm:text-md focus:ring-blue-500 focus:border-blue-500 dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white dark:focus:ring-blue-500 dark:focus:border-blue-500"
		/>
	</div>
	<button
		type="submit"
	...
	>
		{#if loading}
			...
			Loading...
		{:else}
			Récupérer des nouveaux mots de l'IA 🤖️
		{/if}
	</button>
</form>
